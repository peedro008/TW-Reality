...
function Realtors ({google, realtors,Referred}) {
      ...
      useEffect(()=>{
        let pes = []
          realtors?.map((e, index)=>{
            ...
            pes.push(
              [e.name, e.Sells.length, e.Referrals.length, Referred.filter(f=>f.User.id==e.id).length])
          })
          setDato(pes)
      }, [])
  useEffect(() => {
    ...
    if (google && !chart) {
      ...
      const newChart = new google.visualization.ColumnChart(document.getElementById('Realtors'));
  ...
  return (
    <>
      ...
      <div style={{minHeight:"300px", minWidth:"70vw", marginLeft:"-65px",marginTop:"-5px"}} id="Realtors" className={!google ? 'd-none' : ''} ></div>
      }
    </>
  )
}
