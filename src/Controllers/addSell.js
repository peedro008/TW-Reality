...
function AddSell() {
  ...
  const getTransactionsCoord = (UserId) => {
    fetch(
      `https://truewayrealtorsapi.com/getMyTransactionCoordinatorAvailable?id=${UserId}`
    )
      .then((res) => res.json())
  ...
  useEffect(() => {
    let transactions = [];
    myTransactionCoord?.map(
      (e) =>
        (transactions = [
          ...transactions,
          { value: e.id, label: `${e.clientName} - ${e.openDate}` },
        ])
    );
